  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6">
        <h2 className="text-xl font-bold text-gray-100 mb-6">Implementation Timeline</h2>

        {(() => {
          // Filter tasks that have both start and finish dates
          const tasksWithDates = tasks.filter(task => task.start_date && task.finish_date);

          if (tasksWithDates.length === 0) {
            return (
              <div className="text-center py-8 text-gray-400">
                No tasks with start and finish dates yet. Add dates to your tasks to see them on the timeline.
              </div>
            );
          }

          // Find the earliest start date and latest finish date
          const allDates = tasksWithDates.flatMap(t => [new Date(t.start_date), new Date(t.finish_date)]);
          ...
          const getStatusColor = (status: string) => {
            switch (status) {
              case 'Completed': return 'bg-green-600/70 border-green-500';
              case 'In Progress': return 'bg-blue-600/70 border-blue-500';
              case 'Testing': return 'bg-purple-600/70 border-purple-500';
              case 'Next Up': return 'bg-yellow-600/70 border-yellow-500';
              default: return 'bg-gray-600/70 border-gray-500';
            }
          };
          ...
          return (
            <div className="space-y-3">
              ...
              {tasksWithDates.map((task, idx) => {
                const startPos = getPosition(task.start_date);
                const endPos = getPosition(task.finish_date);
                const width = endPos - startPos;

                return (
                  <div key={task.id} className="relative h-12 group">
                    <div
                      className={`absolute h-10 rounded-lg border-2 ${getStatusColor(task.status)} hover:brightness-125 transition-all cursor-pointer flex items-center px-3 overflow-hidden`}
                      style={{
                        left: `${startPos}%`,
                        width: `${width}%`,
                        minWidth: '80px'
                    ...
                    {/* Tooltip on hover */}
                    <div className="absolute left-0 top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 border border-gray-700 rounded-lg p-2 text-xs text-gray-300 whitespace-nowrap z-10 pointer-events-none">
                      <div className="font-semibold text-white">{task.task_name}</div>
                      <div>{new Date(task.start_date).toLocaleDateString()} → {new Date(task.finish_date).toLocaleDateString()}</div>
                      {task.status && <div className="text-gray-400">Status: {task.status}</div>}
                    </div>
                  </div>
                );
              })}
