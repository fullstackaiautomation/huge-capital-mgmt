...
export function SbaSection() {
  const { lenders, loading, error, addLender, updateLender, deleteLender } = useSbaLenders();
  const [editingId, setEditingId] = useState<string | null>(null);
  const [showAddForm, setShowAddForm] = useState(false);
  const [isSaving, setIsSaving] = useState(false);

  const handleAddLender = async (formData: SbaLenderFormData) => {
    try {
      setIsSaving(true);
  ...
  const handleUpdateLender = async (formData: SbaLenderFormData) => {
    if (!editingId) return;
    try {
      setIsSaving(true);
  ...
  const handleEdit = (lender: SbaLender) => {
  ...
  if (loading) {
    return <div className="text-gray-400">Loading SBA lenders...</div>;
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">SBA</h2>
        <button
          ...
          <table className="w-full text-sm text-gray-300">
            <thead className="border-b border-gray-700">
              <tr>
                ...
                <th className="text-left py-3 px-4 font-semibold">States</th>
                ...
                <tr key={lender.id} className="border-b border-gray-700 hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <div className="text-sm">
                      {lender.contact_person && <div>{lender.contact_person}</div>}
                  ...
                  <td className="py-3 px-4 text-xs">{lender.states_available ? lender.states_available.substring(0, 30) + '...' : 'N/A'}</td>
                  <td className="py-3 px-4">
                    {lender.website ? (
                      <a
                        href={lender.website}
